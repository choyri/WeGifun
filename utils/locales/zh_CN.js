const string = {
  changelog: [
    ...
    '修复 同时间段连上课程的判断',
    '修复 连上课程被其他课程遮住',
    '改善 添加自定义课程时可直接设置持续节数',
    '改善 支持 Face ID',
    '改善 页面样式适配 iPhone 全面屏设备',
    '改善 iOS 设备上下拉页面时的窗口背景色',
    '改善 书目检索封面懒加载',
    '改善 移动赞赏入口',
    '改善 细节调整',
  ],

  feature: '🙈 久违的更新~ 新增 & 修复 & 改善 若干问题。',
  ...
  tip: {
    ...
    elec_zero: '余量为零时，可能是当前宿舍不支持查询。',
    guide: '欢迎使用，是否查看使用指南？',
    schedule: '左右滑动可以切换周数，双击回到当前周数，长按进入自定义课表页。',
  ...
  common_guide: {
    content: [
      ...
      '在课表页，左右滑动可以切换周数，双击回到当前周数，长按进入自定义课表页。',
      '点亮课表可以让课表在任意时候都不会变灰。',
